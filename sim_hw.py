...
class Packet:
    ...
    """
    seq_num = 0
    def __init__(self, num_bits, seq_num, data, source_name, time):
        self.num_bits = num_bits
        self.seq_num = Packet.seq_num
        Packet.seq_num += 1
        self.data = data
        self.enqueue_time = None
        self.initial_enqueue_time = time
        self.transmit_time = None
        self.receive_time = None
        self.source_name = source_name
        self.transmit_timeC = None
        self.receive_timeC = None
        self.receive_timeD = None
...
class Node:
    ...
    def receive(self, sim, owner, data):
        ...
        if owner.name == 'D':
            ...
            data.receive_timeC = data.receive_time
...
if __name__ == "__main__":
    """
    Setup a simple topology a --> c , b --> c , c --> d
    """
    node_a = Node('A')
    node_b = Node('B')
    node_c = Node('C')
    ...
    for time in range(0, 10000, 1000):
        for seq in range(10):
            pkt = Packet(1000, seq_num, None, 'A', time)
    ...
    for time in range(0, 10000, 500):
        for seq in range(2):
            pkt = Packet(1000, seq_num, None, 'B', time)
            sim.schedule_event(node_b.enqueue, node_b, pkt, time, 'queue')
            seq_num += 1
    sim.run(10000)

    with open('sim.csv', 'w') as out:
        out.write('Simulation Results: \n' +
                  'seq num, src name, enqueue time @ src, tx time @ src, receive time @ C, tx time @ C, received @ D\n')
    for packet in received_packets:
        queue_delay = packet.transmit_time - packet.enqueue_time
        e2e_delay = packet.receive_time - packet.enqueue_time
        #print('received', packet.enqueue_time, packet.transmit_time, packet.receive_time, e2e_delay, queue_delay)

        simOutput = [
            packet.seq_num,
            packet.source_name,
            packet.initial_enqueue_time,
            packet.enqueue_time,
            packet.receive_timeC,
            packet.transmit_time,
            packet.receive_time
        ]

        output = ",".join(str(x) for x in simOutput)
        with open('sim.csv', 'a') as out:
